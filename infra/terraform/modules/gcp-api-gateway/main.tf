...
resource "google_service_account" "gateway" {
  ...
  account_id   = "corvis-gateway-${var.environment}"
  display_name = "Corvis API Gateway ${var.environment}"
  description  = "Keyless identity used by Google API Gateway to invoke the IAM-private Corvis Cloud Run API."
}
...
resource "google_service_account_iam_member" "api_gateway_service_agent_token_creator" {
...
resource "google_api_gateway_api" "api" {
  provider     = google-beta
  project      = var.project_id
  api_id       = "corvis-api-${var.environment}"
...
resource "google_apikeys_key" "cloudflare_edge" {
  project      = var.project_id
  name         = "corvis-edge-${var.environment}"
  display_name = "Corvis Cloudflare edge ${var.environment}"
...
resource "google_api_gateway_api_config" "api" {
  ...
  api_config_id_prefix = "corvis-${var.environment}-"
  display_name         = "Corvis API ${var.environment}"
  labels               = local.labels
  ...
  depends_on = [
    ...
    google_service_account_iam_member.api_gateway_service_agent_token_creator,
...
resource "google_api_gateway_gateway" "api" {
  ...
  project      = var.project_id
  region       = var.region
  gateway_id   = "corvis-api-${var.environment}"
  display_name = "Corvis API ${var.environment}"
