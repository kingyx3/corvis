...
provider "google" { project = var.project_id region = "asia-southeast1" }
provider "google-beta" { project = var.project_id region = "asia-southeast1" }
provider "cloudflare" {}
...
resource "terraform_data" "edge_configuration_guard" {
  lifecycle { precondition { condition = !local.edge_requested || local.api_runtime_enabled error_message = "Cloudflare edge activation requires an immutable API_IMAGE so API and customer origins cannot point at missing runtimes." } }
}
...
resource "terraform_data" "edge_zone_guard" {
  count = local.edge_enabled ? 1 : 0
  lifecycle { precondition { condition = length(data.cloudflare_zones.corvis[0].result) == 1 && local.cloudflare_zone_id != "" && local.cloudflare_account_id != "" error_message = "CLOUDFLARE_ZONE_NAME must resolve to exactly one active Cloudflare zone with an account ID." } }
}
...
module "cloudflare_edge" {
  ...
  depends_on = [terraform_data.edge_configuration_guard, terraform_data.edge_zone_guard, module.api_gateway]
}
...
module "cloudflare_customer_edge" {
  ...
  depends_on = [terraform_data.edge_zone_guard, module.customer_gateway, module.api_gateway]
}
