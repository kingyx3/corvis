...
export async function GET(request: Request) {
  ...
  try {
    ...
    const rows = await publicServingResources().metricDefinitions();
    const page = paginate(rows, (row) => `${row.metricCode}:${row.definitionVersion}`, parseLimit(url.searchParams.get("limit")), url.searchParams.get("cursor"));
