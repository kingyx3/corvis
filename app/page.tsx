...
type SearchResult =
  ...
  | { kind: "fund"; key: string; title: string; detail: string }
  | { kind: "observation"; key: string; title: string; detail: string };
...
export default function CorvisApp() {
  ...
  const searchResults = useMemo<SearchResult[]>(() => {
    ...
    for (const doc of docs) {
      if (`${doc.name} ${doc.fund} ${doc.period}`.toLowerCase().includes(query)) {
        results.push({ kind: "document", key: `doc:${doc.id}`, title: doc.name, detail: `${doc.fund} · ${doc.period}`, document: doc });
      }
    }
    for (const snapshot of snapshots) {
      if (`${snapshot.fund} ${snapshot.period}`.toLowerCase().includes(query)) {
        results.push({ kind: "fund", key: `fund:${snapshot.id}:${snapshot.period}`, title: snapshot.fund, detail: `${snapshot.period} · ${snapshot.status}` });
      }
    }
    for (const row of observations) {
      if (`${row.company} ${row.metric} ${row.value} ${row.period}`.toLowerCase().includes(query)) {
        results.push({ kind: "observation", key: `obs:${row.id}`, title: `${row.company} · ${row.metric}`, detail: `${row.value} · ${row.period} · ${row.state}` });
      }
    }
  ...
  return <div className="app-shell">
    ...
    {searchOpen && <div role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSearchOpen(false); }} style={{ position: "fixed", inset: 0, zIndex: 80, background: "rgba(15,23,42,.45)", display: "flex", justifyContent: "center", alignItems: "flex-start", paddingTop: "10vh" }}>
      <section role="dialog" aria-modal="true" aria-label="Global workspace search" style={{ width: "min(720px, calc(100vw - 32px))", background: "white", borderRadius: 14, boxShadow: "0 24px 80px rgba(15,23,42,.24)", overflow: "hidden" }}>
        <label style={{ display: "flex", gap: 10, alignItems: "center", padding: 16, borderBottom: "1px solid #e5e7eb" }}><Icon name="search" size={18}/><input autoFocus value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search funds, companies, documents or metrics" aria-label="Search workspace" style={{ flex: 1, border: 0, outline: 0, fontSize: 16 }}/><kbd>Esc</kbd></label>
