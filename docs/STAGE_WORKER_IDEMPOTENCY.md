...
Canonicalization is guarded twice. The reviewed handler returns success only when every candidate satisfies its current policy requirement, and a Postgres trigger independently refuses the `reviewed` job's transition to `succeeded` unless the exact finalized extraction run/candidate-set SHA has a `ready` `candidate_review_v1` gate with zero blockers. Therefore no `canonicalized` job can be created from merely client-supplied state or an unresolved review queue. Canonicalization and all later governed stages remain fail-closed until their own handlers are implemented.
