...
export function ReviewView({ observations, snapshot, onObservationUpdated, onPublished }: {
  ...
  const focused = visible[Math.min(focusedIndex, Math.max(visible.length - 1, 0))];

  useEffect(() => { setFocusedIndex(0); }, [stateFilter, confidenceFilter, query, sortMode]);
  ...
  const applyDecision = async (row: ObservationRecord, decision: "approve" | "reject" | "correct", correctedValue?: string, reasonCode?: string) => {
    ...
    try {
      const outcome = await workspacePort.review({
        observationId: row.id,
        decision,
        reasonCode: reasonCode || (decision === "approve" ? "reviewer_verified" : decision === "reject" ? "reviewer_rejected" : "reviewer_corrected"),
        correctedValue,
  ...
  const openExceptionDialog = (item: ReconciliationException, action: ReconciliationResolutionAction) => {
    setExceptionDialog({ item, action });
    setExceptionNote("");
  };

  const submitExceptionDialog = async () => {
    if (!exceptionDialog) return;
    const dialog = exceptionDialog;
    setExceptionDialog(null);
    await resolveException(dialog.item, dialog.action, exceptionNote);
  };
  ...
  return <>
    ...
    {publishBlocked && snapshot?.id && <div className="lineage-note" role="status"><Icon name="alert"/><div><strong>Publication gate is closed</strong><span>{needsReview} observations need review and {blockingExceptions} reconciliation exceptions remain open.</span></div></div>}
    ...
    {snapshot?.id && <div className="table-card" tabIndex={0} role="region" aria-label="Reconciliation exceptions table"><table className="data-table"><thead><tr><th>Exception</th><th>Context</th><th>Competing evidence</th><th>Status</th><th>Resolution</th></tr></thead><tbody>
      ...
      {exceptions.map((item) => {
        ...
        return <tr key={item.exceptionId}><td><strong>{item.type.replaceAll("_", " ")}</strong><div>{item.summary}</div></td><td>{[item.subjectType, item.subjectId, item.metricCode, item.materiality !== "unknown" ? item.materiality : undefined].filter(Boolean).join(" · ") || "Snapshot-level blocker"}</td><td>{item.sourceReferences.length ? <div className="heading-actions">{item.sourceReferences.map((source) => <button key={source.sourceReferenceId} className="source-link" disabled={busy === `exception-source:${source.sourceReferenceId}`} onClick={() => void showSourceReference(source.sourceReferenceId, `exception-source:${source.sourceReferenceId}`)}><Icon name="source" size={14}/>{source.page ? `Page ${source.page}` : source.sheetName || source.documentId}</button>)}</div> : "No entitled source excerpt available"}</td><td>{item.status === "open" ? <StatusPill status="Needs review"/> : <StatusPill status="Approved"/>}</td><td>{item.status === "open" ? <div className="heading-actions">{item.type === "source_authority" && <select aria-label={`Authoritative source for ${item.summary}`} value={selectedSource} onChange={(event) => setSelectedSources((current) => ({ ...current, [item.exceptionId]: event.target.value }))}><option value="">Choose source</option>{item.sourceReferences.map((source) => <option key={source.sourceReferenceId} value={source.sourceReferenceId}>{source.page ? `Page ${source.page}` : source.documentId}</option>)}</select>}{item.allowedActions.map((action) => <button key={action} className="secondary-button" disabled={busy === `exception:${item.exceptionId}` || (action === "select_source" && !selectedSource)} onClick={() => openExceptionDialog(item, action)}>{actionLabel(action)}</button>)}</div> : <span>Resolved {item.resolvedAt ? new Date(item.resolvedAt).toLocaleString() : ""}</span>}</td></tr>;
      })}
    </tbody></table></div>}

    <div className="toolbar" aria-label="Review filters">
      <label className="search-field"><Icon name="search"/><input aria-label="Search review observations" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search company, metric, value or source"/></label>
      ...
      <select className="filter-button" aria-label="Confidence risk" value={confidenceFilter} onChange={(event) => setConfidenceFilter(event.target.value as typeof confidenceFilter)}><option value="all">All confidence</option><option value="under90">Under 90%</option><option value="under75">Under 75%</option></select>
      <select className="filter-button" aria-label="Review sort" value={sortMode} onChange={(event) => setSortMode(event.target.value as typeof sortMode)}><option value="risk">Risk first</option><option value="confidence">Lowest confidence</option><option value="company">Company / metric</option></select>
      <span className="table-muted" role="status">{visible.length} shown</span>
      <div className="toolbar-spacer"/>
      <button className="text-button" disabled={!visible.length || focusedIndex <= 0} onClick={() => setFocusedIndex((index) => Math.max(0, index - 1))}>Previous</button>
      <button className="text-button" disabled={!visible.length || focusedIndex >= visible.length - 1} onClick={() => setFocusedIndex((index) => Math.min(visible.length - 1, index + 1))}>Next</button>
    </div>
    {focused && <div className="lineage-note" role="status" aria-label="Focused review item"><Icon name="table"/><div><strong>Review queue {Math.min(focusedIndex + 1, visible.length)} of {visible.length}</strong><span>{focused.company} · {focused.metric} · {focused.confidence}% confidence</span></div></div>}
    <div className="table-card" tabIndex={0} role="region" aria-label="Data review observations table"><table className="data-table review-table"><thead><tr><th>Company</th><th>Metric</th><th>Value</th><th>Period</th><th>Change</th><th>Confidence</th><th>Source evidence</th><th>State / action</th></tr></thead><tbody>
      ...
      {visible.map((row, index) => <tr key={row.id} aria-current={index === focusedIndex ? "true" : undefined}><td><strong>{row.company}</strong></td><td>{row.metric}</td><td><strong className="value-cell">{row.value}</strong></td><td>{row.period}</td><td className={row.delta.startsWith("+") ? "positive" : ""}>{row.delta}</td><td><div className="confidence"><span>{row.confidence}%</span><div><i style={{width:`${row.confidence}%`}}/></div></div></td><td><button className="source-link" disabled={!row.sourceReferenceId || busy === `source:${row.id}`} onClick={() => row.sourceReferenceId && void showSourceReference(row.sourceReferenceId, `source:${row.id}`)}><Icon name="source" size={14}/>{row.sourceReferenceId ? row.source : "No entitled source reference"}</button></td><td>{row.state === "Needs review" ? <div className="heading-actions"><button className="secondary-button" disabled={busy === row.id} onClick={() => void applyDecision(row,"approve")}>Approve</button><button className="text-button" disabled={busy === row.id} onClick={() => openReviewDialog(row,"correct")}>Correct</button><button className="text-button" disabled={busy === row.id} onClick={() => openReviewDialog(row,"reject")}>Reject</button></div> : <StatusPill status={row.state}/>}</td></tr>)}
    </tbody></table></div>
    ...
    {reviewDialog && <div role="presentation" style={{ position:"fixed", inset:0, zIndex:90, background:"rgba(15,23,42,.45)", display:"grid", placeItems:"center", padding:20 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setReviewDialog(null); }}><section role="dialog" aria-modal="true" aria-label={`${reviewDialog.decision} observation`} style={{ width:"min(520px,100%)", background:"white", borderRadius:12, padding:22 }}><h2 style={{ marginTop:0 }}>{reviewDialog.decision === "correct" ? "Correct observation" : "Reject observation"}</h2><p>{reviewDialog.row.company} · {reviewDialog.row.metric}</p>{reviewDialog.decision === "correct" && <label style={{ display:"grid", gap:6, marginBottom:14 }}>Corrected value<input autoFocus value={reviewValue} onChange={(event) => setReviewValue(event.target.value)} style={{ padding:10 }}/></label>}<label style={{ display:"grid", gap:6, marginBottom:18 }}>Reason<select value={reviewReason} onChange={(event) => setReviewReason(event.target.value)} style={{ padding:10 }}><option value={reviewDialog.decision === "correct" ? "reviewer_corrected" : "reviewer_rejected"}>{reviewDialog.decision === "correct" ? "Reviewer correction" : "Reviewer rejection"}</option><option value="source_conflict">Source conflict</option><option value="duplicate">Duplicate disclosure</option><option value="out_of_scope">Out of scope</option><option value="other">Other governed reason</option></select></label><div className="heading-actions"><button className="primary-button" disabled={reviewDialog.decision === "correct" && !reviewValue.trim()} onClick={() => void submitReviewDialog()}>Record decision</button><button className="secondary-button" onClick={() => setReviewDialog(null)}>Cancel</button></div></section></div>}

    {exceptionDialog && <div role="presentation" style={{ position:"fixed", inset:0, zIndex:90, background:"rgba(15,23,42,.45)", display:"grid", placeItems:"center", padding:20 }} onMouseDown={(event) => { if (event.target === event.currentTarget) setExceptionDialog(null); }}><section role="dialog" aria-modal="true" aria-label="Resolve reconciliation exception" style={{ width:"min(560px,100%)", background:"white", borderRadius:12, padding:22 }}><h2 style={{ marginTop:0 }}>{actionLabel(exceptionDialog.action)}</h2><p>{exceptionDialog.item.summary}</p><label style={{ display:"grid", gap:6, marginBottom:18 }}>Resolution note<textarea autoFocus rows={4} value={exceptionNote} onChange={(event) => setExceptionNote(event.target.value)} placeholder="Optional evidence or rationale" style={{ padding:10 }}/></label><div className="heading-actions"><button className="primary-button" onClick={() => void submitExceptionDialog()}>Resolve exception</button><button className="secondary-button" onClick={() => setExceptionDialog(null)}>Cancel</button></div></section></div>}
  </>;
}
