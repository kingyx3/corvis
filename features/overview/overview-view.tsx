...
export function OverviewView({
  ...
  return <>
    ...
    <section className="two-column"><div className="panel"><div className="panel-heading"><div><p className="eyebrow">FUND PERIODS</p><h2>Current reporting cycle</h2></div>{canReadDocuments && <button className="text-button" onClick={() => onNavigate("documents")}>View documents <Icon name="arrow" size={15}/></button>}</div><div className="snapshot-list">{snapshots.length ? snapshots.map((item) => canReadObservations ? <button className="snapshot-row" style={{ width:"100%", border:0, background:"transparent", textAlign:"left", cursor:"pointer" }} key={`${item.id || item.fund}-${item.period}`} onClick={() => onSnapshotSelect(item)} aria-label={`Open ${item.fund} ${item.period}`}><div className="fund-mark">{item.fund.split(" ").slice(0,2).map((word) => word[0]).join("")}</div><div className="snapshot-main"><strong>{item.fund}</strong><span>{item.period} · {item.holdings} holdings · {item.facts} facts</span></div><StatusPill status={item.status}/><span className="muted-time">{item.changed}</span><Icon name="chevron" size={16}/></button> : <div className="snapshot-row" key={`${item.id || item.fund}-${item.period}`}><div className="fund-mark">{item.fund.split(" ").slice(0,2).map((word) => word[0]).join("")}</div><div className="snapshot-main"><strong>{item.fund}</strong><span>{item.period} · {item.holdings} holdings · {item.facts} facts</span></div><StatusPill status={item.status}/><span className="muted-time">{item.changed}</span></div>) : <div className="snapshot-row"><div className="snapshot-main"><strong>No snapshots yet</strong><span>{canUpload ? "Upload a source document to start a reporting cycle." : "No entitled fund-period snapshots are available."}</span></div></div>}</div></div><div className="panel activity-panel"><div className="panel-heading"><div><p className="eyebrow">ACTIVITY</p><h2>What changed</h2></div></div><div className="activity-list">{activity.length ? activity.map((item, i) => <div className="activity-row" key={`${item.title}-${item.time}`}><span className={`activity-marker marker-${i}`}></span><div><strong>{item.title}</strong><span>{item.detail}</span></div><time>{item.time}</time></div>) : <div className="activity-row"><div><strong>No recent activity</strong><span>Production activity appears here when audit/read-model events are available.</span></div></div>}</div></div></section>
    {canResearch && <section className="research-callout" onClick={() => onNavigate("research")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onNavigate("research"); }} role="button" tabIndex={0}><div className="research-symbol"><Icon name="spark" size={22}/></div><div><p className="eyebrow">ASK CORVIS</p><h3>What changed in my portfolio this quarter?</h3><p>Query trusted fund data and source documents together, with evidence.</p></div><div className="research-arrow"><Icon name="arrow"/></div></section>}
  </>;
}
