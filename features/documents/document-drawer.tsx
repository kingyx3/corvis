...
export function DocumentDrawer({ doc, onClose, onReview }: { doc: DocumentRecord; onClose: () => void; onReview: () => void }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="document-drawer">
    <div className="drawer-head"><div className="file-tile pdf">{doc.name.endsWith("xlsx") ? "XLS" : "PDF"}</div><button className="icon-button" onClick={onClose}><Icon name="close"/></button></div>
    ...
    {doc.status === "Published" && <><hr/><div className="drawer-actions"><button className="primary-button" onClick={onReview}>Open trusted data <Icon name="arrow" size={15}/></button><button className="secondary-button"><Icon name="source"/>View source</button></div></>}
  </aside></div>;
}
