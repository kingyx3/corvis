...
export function UploadModal({ onClose, onCompleted }: { onClose: () => void; onCompleted: (record: DocumentRecord) => void }) {
  ...
  return <div className="modal-backdrop"><div ref={dialogRef} className="upload-modal" role="dialog" aria-modal="true" aria-labelledby="upload-title" tabIndex={-1}>
    ...
    <button type="button" className={`drop-zone ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => inputRef.current?.click()} aria-label="Choose source documents to upload"><span className="drop-icon" aria-hidden="true"><Icon name="upload" size={24}/></span><strong>Drop files here, or choose files</strong><span>PDF, Excel, Word, PowerPoint or CSV · large files supported</span></button><input ref={inputRef} type="file" hidden multiple accept=".pdf,.xlsx,.xls,.docx,.pptx,.csv" onChange={onChange}/>
    ...
    {items.length > 0 && <div className="upload-list" aria-live="polite">{items.map(([key, item]) => <div className="upload-item" key={key}><div className={`file-tile ${/\.(xlsx|xls|csv)$/i.test(item.fileName) ? "excel" : "pdf"}`} aria-hidden="true">{item.fileName.split(".").pop()?.toUpperCase().slice(0, 4) || "DOC"}</div><div className="upload-item-main"><div><strong>{item.fileName}</strong><span>{formatBytes(item.totalBytes)} · {item.status === "complete" ? "Uploaded" : item.status === "finalizing" ? "Finalizing…" : item.status === "error" ? item.error : `${item.percent}%`}</span></div><div className="upload-progress" role="progressbar" aria-label={`Upload progress for ${item.fileName}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={item.percent}><span style={{width:`${item.percent}%`}}/></div></div>{item.status === "complete" ? <div className="upload-check" role="img" aria-label="Upload complete"><Icon name="check" size={15}/></div> : <span className="upload-percent" aria-hidden="true">{item.status === "error" ? "" : `${item.percent}%`}</span>}</div>)}</div>}
    {showCancelPrompt
