...
export function DocumentsView({ docs, onUpload, onSelect }: { docs: DocumentRecord[]; onUpload: () => void; onSelect: (doc: DocumentRecord) => void }) {
  ...
  return <>
    ...
    <div className="toolbar">
      ...
      <label><span className="sr-only">Reporting period</span><select className="filter-button" aria-label="Reporting period" value={period} onChange={(event) => setPeriod(event.target.value)}><option value="all">All periods</option>{periods.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      <label><span className="sr-only">Document status</span><select className="filter-button" aria-label="Document status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses</option>{statuses.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
      ...
      <span className="table-muted" role="status">{filtered.length} of {docs.length}</span>
    </div>
    <div className="table-card" tabIndex={0} role="region" aria-label="Documents table"><table className="data-table document-table"><thead><tr><th>Document</th><th>Fund / period</th><th>Status</th><th>Quality</th><th>Uploaded</th><th></th></tr></thead><tbody>
      {filtered.length === 0 && <tr><td colSpan={6}>No documents match the current filters.</td></tr>}
      {filtered.map((doc) => <tr key={doc.id} onClick={() => onSelect(doc)}><td><div className="document-cell"><div className={`file-tile ${doc.name.endsWith("xlsx") ? "excel" : "pdf"}`}>{doc.name.endsWith("xlsx") ? "XLS" : "PDF"}</div><div><strong>{doc.name}</strong><span>{doc.type} · {doc.pages} {doc.name.endsWith("xlsx") ? "sheets" : "pages"} · {doc.size}</span></div></div></td><td><strong className="table-primary">{doc.fund}</strong><span className="table-secondary">{doc.period}</span></td><td><StatusPill status={doc.status}/>{doc.status === "Extracting" && <div className="mini-progress"><span style={{width:`${doc.progress ?? 0}%`}}/></div>}</td><td><span className={`quality quality-${doc.quality.toLowerCase()}`}>{doc.quality}</span></td><td className="table-muted">{doc.uploaded}</td><td><Icon name="chevron" size={16}/></td></tr>)}
    </tbody></table></div>
  </>;
}
