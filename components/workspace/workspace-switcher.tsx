...
export function WorkspaceSwitcher({ identity }: { identity: WorkspaceIdentity | null }) {
  ...
  useEffect(() => {
    let active = true;
    void workspacePort.listMyWorkspaces().then((items) => { if (active) setMemberships(items as SwitcherMembership[]); }).catch(() => { if (active) setError("Workspace choices are unavailable. Reload to retry."); });
