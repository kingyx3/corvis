...
async function blockingViolations(page: Page, include?: string): Promise<Violation[]> {
  ...
  return results.violations.filter((violation) =>
    (accessibilityBudget.blockedImpacts as readonly string[]).includes(violation.impact ?? ""),
  );
}

function describe(violations: Violation[]): string {
  return violations
    .map((violation) => `${violation.impact}/${violation.id}: ${violation.help} (${violation.nodes.length} node(s), first: ${JSON.stringify(violation.nodes[0]?.target)})`)
    .join("\n");
}
...
test("primary navigation is reachable and operable by keyboard alone", async ({ page }) => {
  ...
  // Focus order inside the primary navigation must follow the visual order.
  ...
  expect(labels.map((label) => label.replace(/\d+$/, "").toLowerCase())).toEqual([
    "overview",
    "documents",
    "data review",
    "data delivery",
  ...
  // The focused control must activate with the keyboard, not only with a pointer.
...
test("the upload dialog traps focus and restores it on close", async ({ page }) => {
  ...
  // Focus must start inside the dialog and stay inside it while cycling forwards.
  await expect(async () => {
  ...
  for (let index = 0; index < 8; index += 1) {
    ...
    const inside = await page.evaluate(() => !!document.activeElement?.closest('[role="dialog"]'));
    expect(inside, `focus escaped the modal after ${index + 1} Tab press(es)`).toBe(true);
  }
