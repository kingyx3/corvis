...
test("claim reclaims a job orphaned in running once no other delivery holds a live lease (migration 045)", async () => {
  const sql = (await readFile("db/postgres/migrations/045_processing_stage_recovery_hardening.sql", "utf8")).toLowerCase();
  const start = sql.indexOf("create or replace function corvis_control.claim_processing_stage_delivery(");
  assert.ok(start >= 0, "045 must redefine claim_processing_stage_delivery");
...
test("operator retry re-emits the retained stage payload instead of a bare job id (migration 045)", async () => {
  const sql = (await readFile("db/postgres/migrations/045_processing_stage_recovery_hardening.sql", "utf8")).toLowerCase();
  const start = sql.indexOf("create or replace function corvis_control.retry_processing_job(");
  assert.ok(start >= 0, "045 must redefine retry_processing_job");
